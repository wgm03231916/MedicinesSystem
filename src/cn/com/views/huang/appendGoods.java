...
import cn.com.listener.huang.addViewBill_MouseListener;
import cn.com.listener.huang.addViewNewly_MouseListener;
...
public class appendGoods extends JDialog{
	  ...
	  public BuyInView biv;
	  public Purchase p;
//      private JPanel pnlMainN;
//      private JPanel pnlMainS;
      public appendGoods(BuyInView biv){
    	  this.biv=biv;
...
//		row.add(b.getGb().getGood_manufacture());
	...
	// TODO Auto-generated method stub
		...
		Vector row =null;
		List<BigAllBean> list=dao.getBillInfo2();
...
//		pnlMainL.add(pnlMainS,BorderLayout.CENTER);
		...
		lblBillGoods.addMouseListener(new addViewBill_MouseListener(this));
		...
		btnDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(JOptionPane.YES_OPTION==JOptionPane.showConfirmDialog(null, "是否删除该信息", "警告",JOptionPane.YES_NO_OPTION)){
					int a=appendGoods.this.tabView2.getSelectedRow();
					appendGoods.this.dtmView2.removeRow(a);
				}
...
//				 new Purchase(rowIndex,appendGoods.this);
		...
		btnOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				row=new Vector();
				MedicineDaoImp dao=new MedicineDaoImp();
				String name=appendGoods.this.tabView2.getValueAt(0, 0).toString();
				...
				for(BigAllBean b:list){
					...
					row.add(b.getGb().getGood_manufacture());
					row.add(b.getGb().getGoods_id());
					row.add(appendGoods.this.tabView2.getValueAt(0, 6).toString()+"-01");
					row.add(b.getGb().getGoods_setting());
					row.add(appendGoods.this.tabView2.getValueAt(0, 3).toString());
					row.add((b.getGb().getGoods_setting())*(Float.parseFloat(appendGoods.this.tabView2.getValueAt(0, 3).toString())));
         ...
         btnCancel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if(JOptionPane.NO_OPTION==JOptionPane.showConfirmDialog(null, "所选的商品未保存是否保存？", "警告",JOptionPane.YES_NO_CANCEL_OPTION)){
					appendGoods.this.dispose();
				}
				else if(JOptionPane.YES_OPTION==JOptionPane.showConfirmDialog(null, "所选的商品未保存是否保存？", "警告",JOptionPane.YES_NO_CANCEL_OPTION))
				{
					Vector<String> title=new Vector<String>();
					System.out.println(biv);
					title.add("商品名称");
				...
				else if(JOptionPane.CANCEL_OPTION==JOptionPane.showConfirmDialog(null, "所选的商品未保存是否保存？", "警告",JOptionPane.YES_NO_CANCEL_OPTION)){
					int a=appendGoods.this.tabView2.getSelectedRow();
					appendGoods.this.dtmView2.removeRow(a);
				}
         }
